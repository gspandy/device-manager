 ...
 * 
 */
public class Handler3015003 extends Handler {
	private Logger log = LogUtil.getInstance().getLogger(Handler3015003.class);
	...
	public String handleRequest(Map<String, String> title,String deviceRequestXml) {
		...
		if ("3015003".equals(trancode)) {
			try {
				Map<String,String> requestMap = explainDeviceRequestXml(deviceRequestXml);
				String msg = MessagePack.pack0400Info(requestMap);
				...
				String responseMsg = msg.substring(4).trim();
				log.info("0400冲销报文:"+StringUtil.addSpace(msg));
				responseXml = "<?xml version=\"1.0\" encoding=\"GBK\"?><Root>"+HandlerUtil.getHandlerXml(trancode, "0", responseMsg)+"</Root>";
			} catch (Exception e) {
				e.printStackTrace();
				responseXml = HandlerUtil.getErrorXml(trancode, "组装0400冲销报文失败");
			}
		} else {
